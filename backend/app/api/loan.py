...
async def historico_prestamos(usuario_id: int, db: AsyncSession = Depends(get_db)):
...
async def devolver_prestamo(prestamo_id: int, fecha_devolucion: date, db: AsyncSession = Depends(get_db)):
